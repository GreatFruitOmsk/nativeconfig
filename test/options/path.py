...
class MyConfig(DummyMemoryConfig):
    my_path = PathOption('MyPath', env_name='MY_PATH', default=PurePath("."))
...
class TestPathOption(unittest.TestCase, TestOptionMixin):
    ...
    def test_choices_cannot_be_empty(self):
        ...
        with self.assertRaises(InitializationError):
            c.empty_choices = PathOption('EmptyChoices', choices=[], default=PurePath("."))

    def test_default_value_must_be_one_of_choices_if_any(self):
        c = MyConfig.get_instance()
        with self.assertRaises(ValidationError):
            c.invalid_default_choice = PathOption('InvalidDefaultChoice', choices=[PurePath("."), PurePath("/usr/bin")],
                                                  default=".")

    def test_all_choices_must_be_valid(self):
        c = MyConfig.get_instance()
        with self.assertRaises(ValidationError):
            c.invalid_choices = self.OPTION_TYPE('InvalidChoices', choices=[PurePath("."), PurePath("/usr/bin"), "."])
    ...
    def test_value_must_be_one_of_choices_if_any(self):
        class MyConfig(DummyMemoryConfig):
            value_with_choices = self.OPTION_TYPE('ValueWithChoices', choices=[PurePath("."), PurePath("C:\\Windows")])

        with self.assertRaises(ValidationError):
            MyConfig.get_instance().value_with_choices = PurePath("/")

        MyConfig.get_instance().value_with_choices = PurePath(".")
        MyConfig.get_instance().value_with_choices = PurePath("C:\\Windows")
    ...
    def test_deserialize_json(self):
        ...
        self.assertEqual(c.my_path, PurePath("."))

    def test_value_can_be_overridden_by_env(self):
        os.environ['MY_PATH'] = '\"/home/user\"'
        c = MyConfig.get_instance()
        self.assertEqual(c.my_path, PurePath('/home/user'))
    ...
    def test_value_that_cannot_be_deserialized_during_get_calls_resolver(self):
        pass  # all valid unicode strings can be interpreted as PurePath

    def test_invalid_deserialized_value_during_get_calls_resolver(self):
        class Paths(DummyMemoryConfig):
            path = PathOption('Path', choices=[PurePath("/"), PurePath("/home/user")], env_name='PATH_OPTION', default=PurePath("/"))
        ...
        with patch.object(DummyMemoryConfig, 'resolve_value', return_value='unresolved'):
            ...
            my_path = c.path
            self.assertEqual(my_path, PurePath('/home/user'))

    def test_setting_value_resets_one_shot_value(self):
        c = MyConfig.get_instance()
        c.set_one_shot_value_for_option_name('MyPath', '\"/home/user\"')

        c.my_path = PurePath('/home/none')
        self.assertEqual(c.my_path, PurePath('/home/none'))
    ...
    def test_setting_none_deletes_value(self):
        c = MyConfig.get_instance()
        c.my_path = PurePath("C:\\users\\User")
        c.my_path = None
        self.assertEqual(c.my_path, PurePath("."))
    ...
    def test_env_is_first_json_deserialized_then_deserialized(self):
        class Paths(DummyMemoryConfig):
            path = PathOption('Path', choices=[PurePath("C:\\users\\User"), PurePath("/home/user")], env_name='TEST_PATH', default=PurePath("C:\\users\\User"))
        ...
        with patch.object(PathOption, 'deserialize', return_value=PurePath('/home/user')) as mock_deserialize:
            my_path = c.path

        mock_deserialize_json.assert_called_with('\"/home/user\"')
        mock_deserialize.assert_called_with(PurePath('/home/user'))
